#!/bin/bash
# Patches the dev-mode Electron.app Info.plist so the dock shows "C.I.T.Y." instead of "Electron"
...
if [ -d "$ELECTRON_APP" ]; then
  ...
  /usr/libexec/PlistBuddy -c "Set :CFBundleName C.I.T.Y." "$PLIST" 2>/dev/null
  /usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName C.I.T.Y." "$PLIST" 2>/dev/null
